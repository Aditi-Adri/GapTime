...
.grid-container{display:grid;grid-template-columns:45px repeat(7, 1fr);background:var(--color-border-tertiary);gap:1px;border:1px solid var(--color-border-tertiary);border-radius:var(--border-radius-md);overflow:hidden;user-select:none;max-height:300px;overflow-y:auto}
.grid-hd{background:var(--color-background-secondary);padding:6px 0;text-align:center;font-size:11px;font-weight:600;color:var(--color-text-secondary);position:sticky;top:0;z-index:2}
.grid-time{background:var(--color-background-primary);font-size:10px;color:var(--color-text-tertiary);text-align:right;padding-right:4px;border-bottom:1px solid var(--color-border-tertiary);}
.grid-cell{background:var(--color-background-primary);height:18px;cursor:crosshair;border-bottom:1px dashed var(--color-border-tertiary);border-right:1px solid var(--color-border-tertiary);}
.grid-cell:hover{background:var(--color-background-secondary);}
.grid-cell.on{background:rgba(29,158,117,0.15);}
.grid-cell.on-top{border-top:1.5px solid #1D9E75;}
.grid-cell.on-bot{border-bottom:1.5px solid #1D9E75;}
.grid-cell.on-l{border-left:1.5px solid #1D9E75;}
.grid-cell.on-r{border-right:1.5px solid #1D9E75;}
`;
...
export default function ScheduleSync() {
  ...
  const [apiKeyInput, setApiKeyInput] = useState(() => localStorage.getItem('gemini_api_key') || 'AIzaSyBkEST1gRhoEj5lSv2Q28COWGumXMVvFzo');
  ...
  async function handleImage(e) {
    ...
    // Gemini API only supports specific image types
    ...
    if (!supportedTypes.includes(mediaType)) {
      ...
      // Remove any possible markdown blocks from the response
      let clean = txt.replace(/```json/i, '').replace(/```/g, '').trim();
      ...
      if (err.message === 'no_key') setImgError('API Key is required to read schedule from images.');
      else setImgError('Could not read schedule from this image. Please try a clearer photo or use manual entry.');
          ...
          <div className="ov" onClick={closeClassModal}>
            <div className="modal" onClick={e => e.stopPropagation()}>
              ...
              <div style={{ display: 'flex', gap: '8px', marginBottom: '16px' }}>
                ...
                <button className={`mode-btn${classMode === 'grid' ? ' on' : ''}`} onClick={() => { setClassMode('grid'); setParsed([]); setImgError(''); }}>📅 Draw on grid</button>
              ...
              {classMode === 'grid' && <>
                <div style={{ marginBottom: '14px' }}>
                  <div className="grid-container" ref={gridRef} onMouseUp={() => setDragStart(null)} onMouseLeave={() => setDragStart(null)}>
                    <div style={{ background: 'var(--color-background-primary)', position: 'sticky', top: 0, zIndex: 2 }}></div>
                    {DAYS.map(d => (
                      <div key={d} className="grid-hd">{d.slice(0, 3)}</div>
                    ))}
                    
                    {Array.from({length: 28}, (_, i) => i + 16).map(halfHour => {
                      const h = Math.floor(halfHour / 2);
                      const m = halfHour % 2 === 0 ? '00' : '30';
                      const timeStr = `${String(h).padStart(2,'0')}:${m}`;
                      
                      return (
                        <React.Fragment key={halfHour}>
                          <div className="grid-time" style={{ display: 'flex', alignItems: 'center', justifyContent: 'flex-end' }}>{m === '00' ? fmt(timeStr) : ''}</div>
                          {DAYS.map(day => {
                             const ct = timeToMins(timeStr);
                             const nt = ct + 30;
                             const nss = timeToMins(newCls.startTime);
                             const nse = timeToMins(newCls.endTime);
                             
                             const isOnDay = newCls.day === day;
                             const isSelected = isOnDay && ct >= nss && ct < nse;
                             
                             const isTop = isSelected && ct === nss;
                             const isBot = isSelected && nt === nse;
                             
                             let cName = "grid-cell";
                             if (isSelected) {
                                 cName += " on on-l on-r";
                                 if (isTop) cName += " on-top";
                                 if (isBot) cName += " on-bot";
                             }
                             ...
                             return (
                               <div 
                                 ...
                                 className={cName}
                                 onMouseDown={() => {
                                     setDragStart({ day, st: timeStr });
                                     setNewCls(p => ({ ...p, day, startTime: timeStr, endTime: minsToTime(nt) }));
                                 }}
                                 onMouseEnter={(e) => {
                                     if (e.buttons === 1 && dragStart && dragStart.day === day) {
                                         const start = Math.min(timeToMins(dragStart.st), ct);
                                         const end = Math.max(timeToMins(dragStart.st) + 30, nt);
                                         setNewCls(p => ({ ...p, startTime: minsToTime(start), endTime: minsToTime(end) }));
                                     }
                                 }}
                               ></div>
                             );
                ...
                <div style={{ marginBottom: '14px', display: 'flex', gap: '10px', alignItems: 'flex-end' }}>
                  <div style={{ flex: 1 }}>
                    <label className="lbl">Selected Time</label>
                    <div style={{ fontSize: '13px', padding: '8px 11px', background: 'var(--color-background-secondary)', borderRadius: 'var(--border-radius-md)', border: '0.5px solid var(--color-border-secondary)' }}>
                      <strong>{newCls.day}</strong>: {fmt(newCls.startTime)} – {fmt(newCls.endTime)}
                    </div>
                  </div>
                  <div style={{ flex: 1 }}>
                    <label className="lbl">Course name (optional)</label>
                    <input placeholder="e.g. Data Structures…" value={newCls.label} onChange={e => setNewCls(p => ({ ...p, label: e.target.value }))} />
                  </div>
                </div>
                <div style={{ display: 'flex', gap: '8px', justifyContent: 'flex-end' }}>
                  <button className="btn" onClick={closeClassModal}>Cancel</button>
              ...
              {classMode === 'image' && <>
                {!imgLoading && !parsed.length && <>
                  <div style={{ marginBottom: '14px' }}>
                    <label className="lbl">Google Gemini API Key (Free)</label>
                    <input 
                      type="password"
                      placeholder="AIza..." 
                      value={apiKeyInput} 
                    ...
                    />
                    <div style={{ fontSize: '11px', color: 'var(--color-text-secondary)', marginTop: '4px' }}>Get a free key from <a href="https://aistudio.google.com/app/apikey" target="_blank" style={{color: '#1D9E75'}}>Google AI Studio</a>. Saved locally.</div>
                  </div>
                  <div className="drop" onClick={() => apiKeyInput ? fileRef.current?.click() : setImgError('Please enter your API Key first to use the AI image extraction feature.')}>
